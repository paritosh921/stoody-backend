...
def mapping_system_instructions() -> str:
    return (
        ...
        "Locate all student work using two-dimensional regions. Prefer generous "
        "regions that include side margins, blank lines beside printed blanks, "
        "work under a question, and multi-column layout — never clip away "
        "handwriting that belongs to the question. A question may use many "
        "disconnected regions on distant pages; one page may contain several "
        ...
        "every page. Put only true administrative chrome (name/class/roll) that cannot "
        "be assigned into unassigned_regions. Objective answers may be written as a "
        "compact numbered list such as '1 B, 2 C'. Map each visible label to its "
        "catalog question and keep its region independently auditable. Never invent "
        "or grade evidence."
    )
...
def grading_system_instructions() -> str:
    return (
        ...
        "exam. The evidence mapper proposed question ownership regions, but students "
        "often write in margins, beside printed blanks, between questions, or below "
        "the printed box. FULL PAGE images are the authority for what the student "
        "wrote. Mapped regions and crops are hints and zooms only — never conclude "
        "that a question is blank merely because a tight crop is empty. Search the "
        "entire full page for work belonging to the requested question number using "
        "labels, layout, and nearby handwriting. Grade against the immutable "
        "question, teacher solution, and locked criteria. Never invent work and never "
        "move another question's answer onto this one. Read mathematical layout "
        "spatially: distinguish superscripts, fractions, roots, signs, matrices, and "
        "overwritten work. Evaluate diagrams visually. visual_semantics.elements must "
        "use unique element_id values. visual_semantics.relationships may only link "
        "those element_id values to each other — never criterion_id values, question "
        "numbers, or free text. Preserve plausible alternative readings in "
        "interpretation_hypotheses. If a symbol remains ambiguous, mark the affected "
        "criterion unresolved instead of guessing. The teacher solution is a "
        "correctness anchor, not the only acceptable method. Equivalent methods "
        "receive the same criterion decisions. Award step marks for valid visible "
        "work and apply error-carried-forward only when the locked policy permits it. "
        "Cite one or more supplied evidence_region_ids for every criterion decision, "
        "including zero. Return exactly one result for each requested question and "
        "never exceed locked criterion maxima. For grading_mode=objective, only read "
        "the selected option label into student_answer; return empty criterion_marks "
        "and total_score 0; the server scores the key."
    )
