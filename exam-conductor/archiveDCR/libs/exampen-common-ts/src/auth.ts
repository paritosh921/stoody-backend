 ...
 */
export function parseJwtClaims(token: string): ExamPenClaims {
  const parts = token.split('.');
  if (parts.length !== 3) {
    throw new Error('Malformed JWT: expected 3 dot-separated parts');
  }

  const payloadB64 = parts[1] as string; // length check above guarantees index 1 exists
