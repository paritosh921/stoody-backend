...
(one JSON object per newline-terminated line, UTF-8), but adds:

- Client tracking for broadcast (tick / expired events).
- Transport abstraction: Unix domain socket on Linux, TCP loopback on
  Windows (so the test-suite runs everywhere).

This module is intentionally self-contained so that ``hub-timer`` can be
built before ``hub-common`` is packaged as a wheel.
"""
...
class TimerIpcServer:
    ...
    async def start(self) -> None:
        if self._use_tcp:
            ...
            log.info(
                "TimerIpcServer [%s] listening on TCP 127.0.0.1:%d",
            ...
            log.info(
                "TimerIpcServer [%s] listening on %s",
    ...
    async def _on_connection(
        ...
        log.debug("TimerIpcServer: client connected (%d total)", len(self._clients))
        try:
            while True:
                ...
                try:
                    ...
                    log.warning("TimerIpcServer: malformed line ignored")
                    continue
    ...
    async def _dispatch(
        ...
        if handler is None:
            log.warning("TimerIpcServer: no handler for %s", envelope.msg_type)
        ...
        try:
            ...
            log.exception("TimerIpcServer: handler error for %s", envelope.msg_type)
            if envelope.expects_reply:
                err = envelope.make_error(
