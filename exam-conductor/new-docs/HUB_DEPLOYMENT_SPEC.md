...
Parent: exampen-system-design.docx (R1/R2/R3)
...
4. Mobile app queues a server-side binding request: `POST /api/v1/exams/{exam_id}/bindings` to `svc-exam-orch`. Sent immediately if network available, queued otherwise.
...
**Ownership rule:** Hub never creates authoritative bindings. `svc-exam-orch` is the single writable owner. Hub holds provisional records for display and local workflow continuity.
