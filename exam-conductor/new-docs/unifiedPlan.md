...
**Version:** 2.1
**Date:** 2026-03-24
**Status:** DRAFT — TRANSITION DOCUMENT
...
**Document Role:** This file is a *transition plan*, not a spec. It exists to bootstrap the four root specs (`DUAL_MODE_ARCHITECTURE.md`, `PCR_EVAL_ENGINE_SPEC.md`, `LLM_GATE_SPEC.md`, `TAMPER_PROOF_SPEC.md`) and the associated API/event contracts. Once those specs are created and promoted to ACTIVE, this document becomes SUPERSEDED. Implementers must build against the root specs, not against this plan. During the transition (before root specs exist), this document may be consulted for intent and rationale, but any detail it contains is provisional until confirmed in its authoritative spec.

---

## 1. System Identity

ExamPen operates two evaluation pipelines that share a common collection infrastructure but diverge on evaluation depth:

- **DCR (Direct Character Recognition)**: Character-level template mapping for structured exams. Short answers, MCQs, fill-in-the-blank. Strokes are recognized at character level and matched against known answer templates for scoring.
- **PCR (Paginated Character Recognition)**: Deep subjective evaluation for multi-page handwritten responses. Essays, numericals, derivations, diagrams. Responses are OCR'd, segmented per question, classified by content type, and evaluated by LLM with step-level scoring and feedback.

Both modes are controlled through a single LLM Gate that tracks all token usage and enforces budget limits across the entire system.
...
- **DCR and PCR are modes defined by this architecture** — not by any legacy code or backup folders.
...
- **No premature code/packaging decisions** — this document defines contracts, ownership, and storage. Implementation may be embedded in the current backend runtime or deployed separately; that choice is deferred.
...
|-------|----------------------|---------------------|
| Dual-mode framing, shared collection layer, routing | `DUAL_MODE_ARCHITECTURE.md` | `01_SYSTEM_OVERVIEW.md` references it |
| DCR input/output contract, template matching semantics | `DUAL_MODE_ARCHITECTURE.md` §DCR | — |
| PCR pipeline behavior, segmentation, classification, eval flow | `PCR_EVAL_ENGINE_SPEC.md` | Chapters explain context, spec owns detail |
| LLM Gate contract, callers, budget, token storage | `LLM_GATE_SPEC.md` | — |
| Immutability rules, server-side fetch, audit trail | `TAMPER_PROOF_SPEC.md` | — |
...
| State ownership per service | `STATE_OWNERSHIP_MAP.md` | — |
| Failure modes and mitigations | `FAILURE_MITIGATION_REGISTER.md` | Specs reference by ID only |
| Test identifiers | `TEST_SUITE_SPEC.md` | Specs reference by ID only |
...
The RPi hub collects pen strokes identically for both pipelines. The `exam_type` field in exam metadata determines downstream routing.
...
|-------|---------------|
| Raw stroke data | `svc-stroke-ingest` (write-once) |
| HWR recognized text | `svc-dcr-recognizer` |
| Template match result + score | `svc-dcr-scorer` |
...
eval.practice.evaluated      — Practice mode eval complete (for analytics/logging)
...
|----------|---------|---------------|
| `DUAL_MODE_ARCHITECTURE.md` | DCR/PCR mode framing, shared collection layer, routing, DCR contract | DRAFT |
| `PCR_EVAL_ENGINE_SPEC.md` | Full PCR pipeline: segmentation, classification, eval, flags | DRAFT |
| `LLM_GATE_SPEC.md` | Gate contract, allowed callers, budget, token storage | DRAFT |
| `TAMPER_PROOF_SPEC.md` | Immutability, server-side fetch, audit trail rules | DRAFT |
...
| Document | Update |
|----------|--------|
| `DOCUMENT_REGISTRY.md` | Register all new documents with authority rules from §3 |
| `agent_ref_index.md` | Add PCR/DCR/gate task routing to new root specs |
| `STATE_OWNERSHIP_MAP.md` | Add PCR + DCR state ownership entries |
| `FAILURE_MITIGATION_REGISTER.md` | Add PCR failure modes with IDs |
| `TEST_SUITE_SPEC.md` | Add PCR test IDs (U-SEG-*, U-EVAL-*, U-GATE-*, I-EVAL-*, E2E-PCR-*) |
| `COMPONENT_INDEPENDENCE_MAP.md` | Add PCR components to dependency graph |
| `STOODY_INTEGRATION_SPEC.md` | Add PCR integration points, practice mode, feature flag |
...
1. **Pass 1 — Foundation specs**: `DUAL_MODE_ARCHITECTURE.md`, `LLM_GATE_SPEC.md`, `TAMPER_PROOF_SPEC.md`
2. **Pass 2 — PCR detail**: `PCR_EVAL_ENGINE_SPEC.md`, 5 OpenAPI specs, 6 event schemas
...
5. **Pass 5 — Retire this file**: Once all four root specs are ACTIVE, mark `unifiedPlan.md` as SUPERSEDED in DOCUMENT_REGISTRY. It remains readable for historical rationale but is no longer authoritative for any topic.
