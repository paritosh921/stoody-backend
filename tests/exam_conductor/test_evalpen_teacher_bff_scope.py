...
@pytest.mark.asyncio
async def test_teacher_bff_keeps_valid_result_ready_when_latest_reprocess_failed():
    from api.v1.evalpen_teacher_bff_async import get_exam_queue
    ...
    await db["exampen_processing_jobs"].update_one(
        {"submission_id": "SUB-1"},
        {
            "$set": {
                "status": "failed",
                "last_error": "candidate generation timed out",
    ...
    assert result.blocked == []
    assert len(result.ready_to_publish) == 1
    item = result.ready_to_publish[0]
    assert item.processing_status == "failed"
    assert item.processing_error == "candidate generation timed out"
    assert item.status_summary == (
