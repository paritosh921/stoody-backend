...
| `BE-PCR-002` | The whole-copy visual grader reads every original answer page in one request and binds each result to catalog question numbers and submitted source-page numbers. Missing, duplicated, out-of-range, or genuinely unreadable evidence remains unresolved per question; downstream grading must not borrow work from another question. | `exam-conductor/pcr/services/full_document_grading.py`; `objective_answer_sheet.py`; whole-copy grading tests | Model complete, missing, duplicated, unreadable, sideways, and cross-question answers. Validate question IDs, page bounds, criterion IDs, and mark limits server-side. |
