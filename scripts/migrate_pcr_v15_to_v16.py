#!/usr/bin/env python3
"""Inspect or apply the guarded PCR v15 -> v16 contract migration.

The command is dry-run by default. Applying changes the frozen cohort contract,
supersedes prior runs, and queues one fresh durable job per submission. It does
not call the grading model itself.
"""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Dry-run or apply a guarded PCR v15 to v16 migration"
    )
    ...
    parser.add_argument(
        "--requested-by",
        default="operations:pcr-v15-v16-migration",
        help="Audit actor",
    )
    return parser
...
async def run(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        for db_name in tenant_names:
            ...
            plans.extend(await inspect_v15_contracts(
                tenant_db,
                db_name=db_name,
        ...
        for plan in plans:
            ...
            try:
                results.append(await migrate_v15_exam_to_v16(
                    tenant_db,
