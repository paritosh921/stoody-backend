#!/usr/bin/env python3
"""Inspect or apply the guarded PCR v13 -> bounded-evidence v14 migration.

The command is dry-run by default.  Applying requires both ``--apply`` and
the exact confirmation token.  The migration code itself also requires the
token, so a caller cannot accidentally bypass the operator gate by importing
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Dry-run or apply a guarded PCR v13 to v14 cohort migration"
    )
    ...
    parser.add_argument(
        "--requested-by",
        default="operations:pcr-v13-v14-migration",
        help="Audit actor",
    )
    return parser
...
async def run(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        for db_name in tenant_names:
            ...
            plans.extend(await inspect_v13_contracts(tenant_db, db_name=db_name, exam_id=args.exam_id))
